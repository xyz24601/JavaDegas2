...
public class PwrInd implements StatConst
{
  ...
  public void paint(Graphics g, Component c)
  {
    ...
    {
      g.drawImage(jdImages.pInd[i], PIP_WIDTH * i, PIP_TOP, c);
    }

    // blank out if already in use
    if (StatVar.hasBomb)
      g.drawImage(jdImages.pIndBlank, PIP_WIDTH, PIP_TOP, c);
    
    if (StatVar.wType == WeapConst.WT_DOUBLE)
      g.drawImage(jdImages.pIndBlank, PIP_WIDTH * 2, PIP_TOP, c);
    else if (StatVar.wType == WeapConst.WT_LASER)
      g.drawImage(jdImages.pIndBlank, PIP_WIDTH * 3, PIP_TOP, c);

    if (StatVar.maxOption)
      g.drawImage(jdImages.pIndBlank, PIP_WIDTH * 4, PIP_TOP, c);
      
    if (StatVar.hasShield)
      g.drawImage(jdImages.pIndBlank, PIP_WIDTH * 5, PIP_TOP, c);

    // highlight current power location
    if (cPwr != 0)
    {
      g.drawImage(jdImages.pIndLit[cPwr - 1], (int) (PIP_WIDTH * (cPwr - 1)), PIP_TOP, c);
    
      // blank out if already in use
      if ((cPwr == 2) && StatVar.hasBomb)
        g.drawImage(jdImages.pIndBlankLit, PIP_WIDTH, PIP_TOP, c);
      else if ((cPwr == 3) &&
               (StatVar.wType == WeapConst.WT_DOUBLE))
        g.drawImage(jdImages.pIndBlankLit, PIP_WIDTH * 2, PIP_TOP, c);
      else if ((cPwr == 4) &&
               (StatVar.wType == WeapConst.WT_LASER))
        g.drawImage(jdImages.pIndBlankLit, PIP_WIDTH * 3, PIP_TOP, c);
      else if ((cPwr == 5) && StatVar.maxOption)
        g.drawImage(jdImages.pIndBlankLit, PIP_WIDTH * 4, PIP_TOP, c);
      else if ((cPwr == 6) && StatVar.hasShield)
        g.drawImage(jdImages.pIndBlankLit, PIP_WIDTH * 5, PIP_TOP, c);
    }
  }
}
